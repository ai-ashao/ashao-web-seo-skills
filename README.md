...
| [`ai-citation-research`](skills/ai-citation-research/) | Research how a website appears in AI and web-search source sets, then turn source gaps into a small SEO/GEO action plan. | Stable natural-language query sets, source appearance analysis, competitor repetition evidence, opportunity classification, and methodology guardrails. |
...
| [`downloader-opportunity-radar`](skills/downloader-opportunity-radar/) | Finding and validating downloader/save/export SEO niches where young domains rank and the full intent cluster has attributable demand. | Deterministic keyword permutations, traffic sanity checks, a 100-point scorer with age/traffic/policy gates, calibration fixtures, and report templates. |
...
| [`helpful-value-audit`](skills/helpful-value-audit/) | Auditing a utility page, competitor, or SERP for people-first usefulness, functional completeness, reliability, differentiation, ranking moat, and attackability. | A 100-point evidence-aware rubric, TESTED/OBSERVED/CLAIMED/UNKNOWN confidence labels, hard gates, SERP table-stakes analysis, and calibration notes. |
...
| [`technical-seo-audit`](skills/technical-seo-audit/) | Auditing public tool, SaaS, hybrid, or multilingual sites with route-level indexability policy instead of generic SEO thresholds. | Unified Markdown/JSON evidence, bounded SSRF-aware retrieval and site crawl, profile/route classification, crawl-depth and orphan analysis, canonical/internal-link/sitemap consistency, duplicate-template signals, JSON-LD, hreflang, and 60+ regression tests. |
...
```text
Use $downloader-opportunity-radar to find downloader niches with young-domain ranking proof and attributable intent-cluster demand.
```
...
```text
Use $helpful-value-audit to audit this tool page for the query "compress image to 100kb" and separate Helpful Strength from Ranking Moat.
```
