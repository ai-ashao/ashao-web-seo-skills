...
def _selected_rules(groups: list[dict[str, object]], user_agent: str) -> tuple[list[dict[str, str]], list[str]]:
    ...
    selected_groups, selected_tokens = [], []
    ...
    return [rule for group in selected_groups for rule in group["rules"]], selected_tokens
...
def evaluate_robots(groups: list[dict[str, object]], user_agent: str, path: str) -> dict[str, object]:
    ...
    matched = max(matches, key=lambda rule: (len(rule["pattern"].replace("*", "").rstrip("$")), rule["directive"] == "allow")) if matches else None
    ...
    return {"selected_agents": selected_tokens, "rules": rules, "allowed": allowed, "matched_rule": matched, "site_wide_block": broad_disallow and not exceptions}
...
def robots_result(target_url: str, timeout: int) -> tuple[dict[str, object], list[str]]:
    ...
    if result.error:
        return {"status": "warn", "severity_hint": "high", "detail": result.error, "url": result.url}, []
    if result.status_code == 404:
        return {"status": "info", "severity_hint": "none", "detail": "No robots.txt at the standard path; this alone does not block crawling.", "url": result.url}, []
    if result.status_code == 429 or (result.status_code is not None and result.status_code >= 500):
        return {"status": "warn", "severity_hint": "high", "http_status": result.status_code, "detail": f"robots.txt returned HTTP {result.status_code}; crawler access can be temporarily suppressed. Re-check persistence.", "url": result.url}, []
    if result.status_code != 200:
        return {"status": "review", "severity_hint": "medium", "http_status": result.status_code, "detail": f"robots.txt returned HTTP {result.status_code}.", "url": result.url}, []
    groups, sitemap_urls = parse_robots(result.body or "")
    parsed_target = urlsplit(target_url)
    target_path = parsed_target.path or "/"
    if parsed_target.query: target_path += f"?{parsed_target.query}"
    ...
    return {
        "status": "fail" if target_blocked or site_wide_block else "pass", "severity_hint": "blocker" if site_wide_block else "high" if target_blocked else "none",
        ...
        "url": result.url, "target_path": target_path, "target_url_blocked": target_blocked, "site_wide_block": site_wide_block,
        "googlebot": googlebot, "wildcard": wildcard, "sitemap_directives": sitemap_urls,
    }, sitemap_urls
...
def _loc_values(root: element_tree.Element, sitemap_type: str) -> list[str]:
    ...
    values = []
    for entry in root:
        if _local_name(entry) != entry_name: continue
        for element in entry:
            if _local_name(element) == "loc" and element.text and element.text.strip(): values.append(element.text.strip())
    return values
...
def parse_sitemap_document(content: str, source_url: str, target_url: str) -> dict[str, object]:
    ...
    return {
        "status": "review" if not locations else "warn" if invalid else "pass", "type": sitemap_type,
        "entry_count": len(locations), "unique_entry_count": len(unique), "duplicate_count": len(locations)-len(unique),
        "invalid_locations": invalid, "foreign_origins": foreign_origins, "target_url_included": target_included,
        "locations": unique, "detail": "Sitemap XML is valid but contains no usable url/sitemap entries." if not locations else "Sitemap contains invalid loc values." if invalid else "Sitemap parsed; inclusion and counts are observations, not index coverage guarantees.",
    }
...
def sitemap_result(origin: str, target_url: str, sitemap_urls: list[str], timeout: int, max_sitemaps: int) -> dict[str, object]:
    ...
    while queue and len(documents) < document_limit:
        ...
        if parsed.get("type") == "sitemapindex":
            for child in parsed.get("locations", []):
                ...
                if parsed_child.scheme not in {"http", "https"} or not parsed_child.netloc or child in queued: continue
                queued.add(child)
                if len(documents) + len(queue) < document_limit: queue.append(child)
                else: bounded_out += 1
    ...
    status = "warn" if warnings else "review" if reviews else "pass" if valid else "warn" if declared else "info"
    queued_remaining = len(queue)
    page_urls = []
    for document in documents:
        if document.get("type") == "urlset": page_urls.extend(document.get("locations", []))
    return {
        "status": status, "declared": declared, "documents_checked": len(documents), "documents_queued_remaining": queued_remaining,
        "documents_bounded_out": bounded_out, "documents_not_checked": queued_remaining+bounded_out, "target_url_included": target_included,
        "page_urls": list(dict.fromkeys(page_urls)), "documents": documents,
        "detail": "Some sitemap documents were valid XML but empty; review intent." if reviews and not warnings else "Sitemap documents checked within the configured bound; target absence is review-only." if valid else "No valid sitemap document was retrieved within the configured locations/bound.",
    }
